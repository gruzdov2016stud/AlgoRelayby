...
public class RDIR extends LN {
    ...
    public void process() {
        ...
        float ang = (float) Math.toRadians(
                SeqV.getС3().getInstCVal().getMag().getValue() - SeqA.getС3().getInstCVal().getMag().getValue());
        /*Расчет sin*/
        float sinS = (float) Math.sin(ang);
        /*Расчет мощности нулевой последовательности*/
        float TotVA0 = SeqA.getС3().getInstCVal().getMag().getValue() *
                SeqV.getС3().getInstCVal().getMag().getValue()*sinS;
