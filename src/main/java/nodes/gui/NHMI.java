...
public class NHMI extends LN {
	...
	public void process(){
		...
		currentTime += 1;
		datasets.forEach((series, DataAttribute) -> {
			if(DataAttribute.getValue() instanceof Number) series.add(currentTime, (Number) DataAttribute.getValue(), false);
			else if(DataAttribute.getValue() instanceof Boolean) series.add(currentTime, (Boolean) DataAttribute.getValue() ? 1 : 0, false);
		});
